...
class SchemaGeneratorService:
    ...
    def __init__(self, db_config: Dict[str, Any], openai_api_key: str):
        ...
        self.provider = ai_cfg.get("active_provider", "openai")
        self.model = ai_cfg.get("chat_model", "gpt-5.2")
        ...
        if self.provider == "groq" and groq_key:
            try:
                ...
                logger.warning(f"Failed to init Groq for schema generation: {exc}")
...
Generate a similar description for {table_name}:"""
        ...
        try:
            max_tokens = 300 if use_extended_thinking else 150
            logger.info(f"   🤖 Generating AI description with {self.provider}:{self.model}...")
            ...
            if self.provider == "groq" and self.groq_client:
                ...
                raise RuntimeError("No AI client available for schema description generation")
