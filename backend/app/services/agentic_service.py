...
class AgenticService:
    ...
    def __init__(self):
        ...
        self._active_provider = cfg.get("active_provider", "openai")
        self._active_agent_model = cfg.get("agent_model", "gpt-5.2")
    ...
    def _initialize_langchain_llm(self):
        ...
        try:
            config = get_ai_config_service().get_config()
            configured_provider = config.get("active_provider", "openai")
            configured_model = config.get("agent_model", "gpt-5.2")

            # Get configured provider keys
            ...
            if configured_provider == "openai":
                if not openai_key:
                    raise ValueError("❌ OPENAI_API_KEY is not set but OpenAI is selected in AI Config!")
                ...
                logger.info(f"✅ Using OpenAI LLM for agents ({configured_model})")
                return llm
            
            elif configured_provider == "groq":
                if not groq_key:
                    raise ValueError("❌ GROQ_API_KEY is not set but Groq is selected in AI Config!")
                ...
                logger.info(f"✅ Using Groq LLM for agents ({configured_model})")
                return llm

            # If configured provider not recognized, try available keys
            logger.warning(f"⚠️ Unrecognized provider '{configured_provider}', trying available keys")
            if openai_key:
                from langchain_openai import ChatOpenAI
                llm = ChatOpenAI(api_key=openai_key, model="gpt-5.2", temperature=0.7, max_tokens=2000)
                logger.info("✅ Using fallback OpenAI LLM for agents (gpt-5.2)")
                return llm
            if groq_key:
                from langchain_groq import ChatGroq
                llm = ChatGroq(api_key=groq_key, model="llama-3.3-70b-versatile", temperature=0.7, max_tokens=2000)
                logger.info("✅ Using fallback Groq LLM for agents (llama-3.3-70b-versatile)")
                return llm
            
            # If no API available, use mock
            logger.error("❌ No LLM API available for agents. Set GROQ_API_KEY or OPENAI_API_KEY in environment")
            return None
...
"""))
    ...
    def _refresh_agentic_llm_if_needed(self):
        """Reinitialize LangChain LLM/graph if admin AI config changed."""
        cfg = get_ai_config_service().get_config()
        provider = cfg.get("active_provider", "openai")
        model = cfg.get("agent_model", "gpt-5.2")

        if provider == self._active_provider and model == self._active_agent_model:
            return

        self._active_provider = provider
        self._active_agent_model = model
        self.llm = self._initialize_langchain_llm()
        self._graph = None
        self._graph_initialized = False
        logger.info(f"🔄 Agentic LLM refreshed: provider={provider}, model={model}")
    ...
    def process_query(self, chat_request: ChatRequest) -> ChatResponse:
        ...
        """
        try:
            self._refresh_agentic_llm_if_needed()
