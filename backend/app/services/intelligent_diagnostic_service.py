...
{{
    ...
    def diagnose_problem(self, chat_request: ChatRequest) -> ChatResponse:
        """
        Main diagnostic workflow with self-improving loop:
        0. Classify user intent to determine response format
        1. Understand the problem
        2. Check historical issues
        3. Generate diagnostic queries
        4. Execute and analyze
        5. Search documentation
        6. Synthesize solution (with dynamic formatting)
        7. LOOP: Judge evaluates and suggests improvements
        8. Return best diagnosis found
        """
        try:
            problem = chat_request.message
            logger.info(f"🔍 Starting intelligent diagnosis with self-improving loop: {problem[:80]}...")
            
            # Step 0: Classify user intent to determine response format
            ...
            # Step 3: Generate initial diagnostic SQL queries
            ...
            for refinement_iteration in range(1, self.max_refinement_iterations + 1):
                ...
                # Step 6: Synthesize solution from current data with dynamic formatting
                ...
                # Step 7: Judge evaluates diagnosis quality
...
{{
    ...
    def _synthesize_solution(self, problem: str, problem_analysis: Dict, 
                                   ...
                                   doc_context: str, intent_classification: Dict[str, Any] = None) -> Dict[str, Any]:
