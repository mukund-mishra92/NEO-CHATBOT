...
class ChatHistoryService:
    ...
    def _ensure_tables_exist(self):
        ...
        try:
            ...
            # User-specific session metadata (custom names for sessions)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS chatbot_session_metadata (
                    id BIGINT AUTO_INCREMENT PRIMARY KEY,
                    session_id VARCHAR(100) NOT NULL,
                    ...
                    INDEX idx_session_meta_session (session_id),
                    INDEX idx_session_meta_user (user_id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
            """)
    ...
    def get_user_chat_sessions(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        Sessions are grouped by session_id so mixed KB/SQL conversations appear
        as one unified session in history.
        """
        try:
            conn = self._get_connection()
            ...
            cursor.execute("""
                SELECT
                    ...
                    CASE
                        WHEN COUNT(DISTINCT chatbot_type) > 1 THEN 'mixed'
                        ELSE MIN(chatbot_type)
                    END AS chatbot_type,
                    MIN(ch.timestamp) AS started_at,
                    MAX(ch.timestamp) AS last_message_at,
                    COUNT(*) AS message_count,
                    SUBSTRING(MIN(CONCAT(LPAD(ch.id, 20, '0'), ch.user_query)), 21) AS first_query,
                    MAX(sm.session_name) AS session_name
                FROM chatbot_chat_history ch
                LEFT JOIN chatbot_session_metadata sm
                    ON sm.session_id = ch.session_id
                    AND sm.user_id = ch.user_id
                WHERE ch.user_id = %s
    ...
    def delete_session(self, session_id: str) -> int:
        ...
        try:
            ...
            # Clean up optional per-user session metadata.
            cursor.execute(
                "DELETE FROM chatbot_session_metadata WHERE session_id = %s",
                (session_id,)
            )
    ...
    def rename_session(self, session_id: str, user_id: str, session_name: str) -> bool:
        """
        Create or update a custom session name for the given user/session.
        """
        try:
            ...
            cursor.execute(
                """
                INSERT INTO chatbot_session_metadata (session_id, user_id, session_name)
                VALUES (%s, %s, %s)
                ON DUPLICATE KEY UPDATE
                    session_name = VALUES(session_name),
                    updated_at = CURRENT_TIMESTAMP
                """,
                (session_id, user_id, clean_name)
            )
            ...
            logger.info(f"✏️ Renamed session {session_id} for user {user_id}")
            return True
