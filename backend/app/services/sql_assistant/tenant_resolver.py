    ...
    def _load_or_create_embeddings(self):
        ...
        if self.embeddings_path.exists() and self.metadata_path.exists():
            try:
                ...
                deprecated_tenants = {"SITE_A", "SITE_B", "SITE_C", "SITE_D"}
                cached_tenants = {m.get("tenant_id") for m in self.tenant_metadata or []}
                if cached_tenants & deprecated_tenants:
                    logger.info("♻️ Deprecated placeholder tenants found in cache. Rebuilding tenant embeddings.")
