...
class SQLAssistantService:
    ...
    def _replace_parameters_in_reused_sql(
        self, sql: str, original_query: str, new_query: str,
        entities: dict = None
    ) -> str:
        """Replace date/time/entity/interval parameters in reused SQL with values from the new query."""
        try:
            from dateutil import parser as dateutil_parser
        except ImportError:
            logger.warning("python-dateutil not installed: date substitution unavailable")
            return sql

        original_lower = original_query.lower().strip()
        new_lower = new_query.lower().strip()

        if original_lower == new_lower:
            return sql

        modified_sql = sql
        dates_replaced = False

        # ------------------------------------------------------------------
        # 1) Replace absolute dates (NL extraction → SQL replacement)
        # ------------------------------------------------------------------
        date_pattern = (
        ...
        orig_dates = re.findall(date_pattern, original_lower)
        new_dates = re.findall(date_pattern, new_lower)

        for old_date, new_date in zip(orig_dates, new_dates):
            if old_date != new_date:
                try:
                    old_dt = dateutil_parser.parse(old_date, dayfirst=True)
                    new_dt = dateutil_parser.parse(new_date, dayfirst=True)
                    old_sql = old_dt.strftime("%Y-%m-%d")
                    new_sql = new_dt.strftime("%Y-%m-%d")
                    if old_sql in modified_sql:
                        modified_sql = modified_sql.replace(old_sql, new_sql)
                        dates_replaced = True
                        logger.info(f"🔄 Replaced date {old_sql} -> {new_sql}")
                except Exception:
                    pass

        # ------------------------------------------------------------------
        # 1b) Fallback: use entities["requested_dates"] + SQL date extraction
        #     Handles cases where NL regex fails to match dates in queries
        # ------------------------------------------------------------------
        if not dates_replaced and entities:
            new_dates_iso = entities.get("requested_dates", [])
            if new_dates_iso:
                # Extract YYYY-MM-DD dates present in the SQL
                sql_date_matches = re.findall(r"'(\d{4}-\d{2}-\d{2})", modified_sql)
                # De-duplicate preserving order
                seen = set()
                unique_sql_dates = []
                for d in sql_date_matches:
                    if d not in seen:
                        seen.add(d)
                        unique_sql_dates.append(d)

                for old_sql_date, new_iso_date in zip(unique_sql_dates, new_dates_iso):
                    if old_sql_date != new_iso_date:
                        modified_sql = modified_sql.replace(old_sql_date, new_iso_date)
                        dates_replaced = True
                        logger.info(f"🔄 Replaced date (entity fallback): {old_sql_date} -> {new_iso_date}")

        # ------------------------------------------------------------------
        # 2) Replace times in SQL using entities["requested_times"]
        # ------------------------------------------------------------------
        if entities:
            new_times = entities.get("requested_times", [])
            if new_times:
                # Extract times from the original query
                time_pattern = r"\b(\d{1,2}:\d{2})\s*(?:am|pm)?\b|\b(\d{1,2})\s*(am|pm)\b"
                orig_time_matches = re.findall(time_pattern, original_lower)
                orig_times_parsed = []
                for m in orig_time_matches:
                    raw = m[0] if m[0] else f"{m[1]}:00 {m[2]}"
                    try:
                        dt = dateutil_parser.parse(raw, fuzzy=True)
                        orig_times_parsed.append(dt.strftime("%H:%M"))
                    except Exception:
                        pass

                for old_time, new_time in zip(orig_times_parsed, new_times):
                    if old_time != new_time:
                        # Replace in SQL: match HH:MM or HH:MM:SS
                        old_time_full = old_time + ":00"
                        new_time_full = new_time + ":00"
                        if old_time_full in modified_sql:
                            modified_sql = modified_sql.replace(old_time_full, new_time_full)
                            logger.info(f"🔄 Replaced time {old_time_full} -> {new_time_full}")
                        elif old_time in modified_sql:
                            modified_sql = modified_sql.replace(old_time, new_time)
                            logger.info(f"🔄 Replaced time {old_time} -> {new_time}")

        # ------------------------------------------------------------------
        # 3) Replace relative intervals (INTERVAL N DAY/MONTH/YEAR)
        # ------------------------------------------------------------------
        interval_pattern = r'(?:last|past)\s+(\d+)\s+(days?|weeks?|months?|years?)'
        orig_interval = re.search(interval_pattern, original_lower)
        new_interval = re.search(interval_pattern, new_lower)

        if orig_interval and new_interval:
            old_num = orig_interval.group(1)
            old_unit = orig_interval.group(2).rstrip('s').upper()  # day/month/year
            new_num = new_interval.group(1)
            new_unit = new_interval.group(2).rstrip('s').upper()

            if old_num != new_num or old_unit != new_unit:
                # Replace INTERVAL N UNIT in SQL (MySQL syntax)
                sql_interval_pattern = rf'INTERVAL\s+{re.escape(old_num)}\s+{re.escape(old_unit)}'
                replacement = f'INTERVAL {new_num} {new_unit}'
                modified_sql_new = re.sub(
                    sql_interval_pattern, replacement, modified_sql, flags=re.IGNORECASE
                )
                if modified_sql_new != modified_sql:
                    modified_sql = modified_sql_new
                    logger.info(f"🔄 Replaced interval: {old_num} {old_unit} -> {new_num} {new_unit}")

        # ------------------------------------------------------------------
        # 4) Replace entity IDs (bot/station/wave/order/bin)
        # ------------------------------------------------------------------
        entity_patterns = {
            "bot": (r"\bbot\s*[-_ ]?\s*(\d{1,4})\b", "BOT-{num:04d}"),
            "station": (r"\bstation\s*[-_ ]?\s*(\d{1,4})\b", "STATION-{num:04d}"),
            "wave": (r"\bwave\s*[-_ ]?\s*(\d{1,6})\b", "WAVE-{num:06d}"),
            "order": (r"\border\s*[-_ ]?\s*(\d{1,6})\b", "ORD-{num:06d}"),
            "bin": (r"\bbin\s*[-_ ]?\s*(\d{1,6})\b", "BIN-{num:04d}"),
        }

        for key, (pattern, canonical) in entity_patterns.items():
            orig_matches = re.findall(pattern, original_lower)
            new_matches = re.findall(pattern, new_lower)
            for old_num, new_num in zip(orig_matches, new_matches):
                if old_num != new_num:
                    old_val = canonical.format(num=int(old_num))
                    new_val = canonical.format(num=int(new_num))
                    if old_val in modified_sql:
                        modified_sql = modified_sql.replace(old_val, new_val)
                        logger.info(f"🔄 Replaced entity {old_val} -> {new_val}")
    ...
    def process_query(self, request):
        ...
        if reused:
            sql, execution_result, original_user_query = reused

            # 🔥 Substitute changed parameters (dates, times, intervals, entity IDs)
            sql = self._replace_parameters_in_reused_sql(
                sql, original_user_query, clean_question, entities=entities
            )
            ...
            # Re-execute with substituted SQL (with archive table support)
            # Extract tables from the reused query if not available
            reused_tables = []
            try:
                # Try to extract table names from the reused query
                import re
                table_pattern = r'\bFROM\s+(\w+)\b|\bJOIN\s+(\w+)\b'
                matches = re.findall(table_pattern, sql, re.IGNORECASE)
                reused_tables = [m[0] or m[1] for m in matches if m[0] or m[1]]
            except:
                pass

            execution_result = self.executor.execute(
                sql,
                enable_archive_detection=True,
                question=clean_question,
                tables_used=reused_tables if reused_tables else None
            )

            # Update SQL to reflect archive UNION if executor modified it
            if execution_result.executed_sql:
            ...
            def generate_fn(feedback, previous_sql):
                ...
                for key, value in entities.items():
                    ...
                    # Skip optional extracted-but-unused entities (like requested_times, requested_dates)
                    if key in ['requested_times', 'requested_dates']:
                        continue
                    
                    # Handle list values
                    if isinstance(value, list):
                        # Skip validation if list is empty (no values extracted)
                        if value and not any(str(v) in sql for v in value):
            ...
            # 🔥 CRITICAL: re-execute corrected SQL with archive table support
            execution_result = self.executor.execute(
                sql,
                enable_archive_detection=True,
                question=clean_question,
    ...
    def _try_kpi_match(self, clean_question: str, entities: dict, session_id: str):
        ...
        # Extract table names from KPI query
        kpi_tables = []
        try:
            import re
            table_pattern = r'\bFROM\s+(\w+)\b|\bJOIN\s+(\w+)\b'
            matches = re.findall(table_pattern, sql, re.IGNORECASE)
            kpi_tables = [m[0] or m[1] for m in matches if m[0] or m[1]]
        except:
            pass

        # Execute the KPI query with archive table support
        sql = kpi_match.sql
        execution_result = self.executor.execute(
            sql,
            enable_archive_detection=True,
            question=clean_question,
            tables_used=kpi_tables if kpi_tables else None
        )
