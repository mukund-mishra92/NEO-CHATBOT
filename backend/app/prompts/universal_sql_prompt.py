...
================================================================================
ARCHIVE TABLE SUPPORT FOR HISTORICAL DATA
================================================================================

CRITICAL: Many tables in this database have corresponding "_archive" tables that
store historical/completed data. When users ask for data spanning a date range that
may include old records, you MUST be aware of archive tables.

ARCHIVE TABLE PAIRS:
- bot_alarm_log ↔ bot_alarm_log_archive
- pick_wave_order_master ↔ pick_wave_order_master_archive
- put_wave_order_master ↔ put_wave_order_master_archive
- steps ↔ steps_archive
- wave_master ↔ wave_master_archive
- And others (see schema context for complete list)

WHEN TO USE ARCHIVE TABLES:
1. User asks for data from the "last N days" or specific date range
2. The backend will check: Does this date range EXTEND beyond the main table's data?
3. If YES → the generated query will be AUTOMATICALLY MODIFIED by the backend
4. Archive data is UNION'ed with main table to provide complete results

YOUR ROLE IN ARCHIVE QUERIES:
- DO NOT explicitly UNION with archive tables in your generated SQL
- The backend will handle this automatically AFTER query generation
- Instead, write your query targeting the MAIN table only
- Include proper date filtering using the timestamp column for the table
- The backend will detect date ranges and add archive UNION if needed

ARCHIVE TABLE TIMESTAMP COLUMNS (for date range filtering):
- bot_alarm_log / bot_alarm_log_archive: INSERTED_TIMESTAMP
- pick_wave_order_master / pick_wave_order_master_archive: logged_timestamp
- put_wave_order_master / put_wave_order_master_archive: logged_timestamp
- steps / steps_archive: logged_timestamp
- wave_master / wave_master_archive: WAVE_CREATION_TIMESTAMP

ARCHIVE QUERY EXAMPLE (You generate this — backend adds UNION):

USER: "Give me top SKUs from last 7 days"

❌ DON'T WRITE:
   SELECT skbm.SKU_ID, COUNT(*) as count
   FROM sku_batch_master skbm
   UNION ALL
   SELECT skbm.SKU_ID, COUNT(*) as count
   FROM sku_batch_master_archive skbm
   WHERE DATE(skbm.CREATED_DATE) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)

✅ WRITE THIS (backend handles archive):
   SELECT skbm.SKU_ID, COUNT(*) as sku_count
   FROM sku_batch_master skbm
   WHERE DATE(skbm.logged_timestamp) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
     AND skbm.`host-location` = ?
   GROUP BY skbm.SKU_ID
   ORDER BY sku_count DESC
   LIMIT 20;

BACKEND PROCESS:
   Step 1: Recognizes date range in query (last 7 days)
   Step 2: Queries MIN/MAX dates from sku_batch_master
   Step 3: Compares: "Is 7 days ago BEFORE main table's minimum date?"
   Step 4: If YES → Automatically UNIONs the same query against archive table
   Step 5: Returns combined results from both tables

KEY POINTS:
✅ Write timestamp filters for the timestamp column of the table being queried
✅ Include proper WHERE conditions (host-location, IS_ACTIVE, etc.)
✅ Use DATE() function for date comparisons
✅ Let backend handle archive UNION logic automatically
✅ Keep queries deterministic and focused on main table
❌ Do NOT hardcode archive table names in your query
❌ Do NOT write UNION ALL in your query
❌ Do NOT duplicate tables in joins just to avoid archive tables
