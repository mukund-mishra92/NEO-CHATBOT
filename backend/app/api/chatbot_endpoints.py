...
class SessionRenameRequest(BaseModel):
    """Payload for renaming a user session."""
    session_name: str
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if not session:
            # Use one unified session type so KB + SQL share a single timeline/context.
        ...
        # Ensure services receive unified server-side history.
...
@router.put("/user/{user_id}/session/{session_id}/rename")
async def rename_user_session(user_id: str, session_id: str, payload: SessionRenameRequest):
    """
    ...
    try:
        ...
        new_name = (payload.session_name or "").strip()
        if not new_name:
            raise HTTPException(status_code=400, detail="Session name cannot be empty")
        if len(new_name) > 120:
            raise HTTPException(status_code=400, detail="Session name is too long (max 120 characters)")

        renamed = chat_history_service.rename_session(
            session_id=session_id,
            user_id=user_id,
...
async def create_new_session(session_type: str = "general"):
    ...
    """
    try:
        # Keep session_type query param for backward compatibility, but use one
        # unified general session for all assistant types.
        session_id = session_manager.create_session(session_type=SessionType.GENERAL)
        
        logger.info(
            f"🆕 Created new unified session: {session_id} "
            f"(requested_type={session_type})"
        )
